...
namespace ProjectWasel.Models.Repositres
{
    public class ReportRepository : IReportRepository
    {
            ...
            string? sortBy = null)
        {
            ...
            // ⚠️ عندك ما عندك enum status في Report الحالي
            // فبنخليه string compare لو بدك تضيفه لاحقًا
            if (!string.IsNullOrWhiteSpace(status))
            {
                query = query.Where(r => r.Category == status);
            }

            // Sorting
            query = sortBy switch
            {
                "votes" => query.OrderByDescending(r => r.Votes),
        ...
        public async Task<Report?> FindDuplicateAsync(
            double lat, double lng, string category, int radiusMeters = 500)
        {
        ...
        public async Task UpdateVoteCountsAsync(int reportId)
        {
            ...
            report.Votes = await _context.ReportVotes
