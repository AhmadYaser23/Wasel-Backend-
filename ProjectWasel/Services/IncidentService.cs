...
namespace ProjectWasel.Services
{
    public class IncidentService
    {
        ...
        // ── Plain list (kept for backward compat) ───────────────────────────────
        ...
        public Task<List<Incident>> GetByCheckpointRawAsync(int checkpointId) =>
            _incidentRepo.GetByCheckpointRawAsync(checkpointId);

        public Task<List<Incident>> GetFilteredAsync(string? type, string? severity) =>
            _incidentRepo.GetFilteredAsync(type, severity);

        // ── Paged + filtered + sorted ───────────────────────────────────────────
        public Task<PagedResult<Incident>> GetPagedAsync(IncidentQueryParams queryParams) =>
            _incidentRepo.GetPagedAsync(queryParams);
        ...
        public Task<Incident?> UpdatePartialAsync(int id, IncidentUpdateDTO dto) =>
            _incidentRepo.UpdatePartialAsync(id, dto);

        public Task<Incident?> VerifyAsync(int id, int verifiedByUserId) =>
            _incidentRepo.VerifyAsync(id, verifiedByUserId);
