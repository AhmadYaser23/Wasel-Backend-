    ...
    public class CheckpointRepository : GenericRepository<Checkpoint>, ICheckpointRepository
    {
        ...
        public async Task<bool> DeleteAsync(int id)
        {
            var checkpoint = await _context.Checkpoints
                ...
                .FirstOrDefaultAsync(c => c.CheckpointId == id);

            if (checkpoint == null) return false;

            // حذف الابناء أولاً
            _context.Incidents.RemoveRange(checkpoint.Incidents);
            _context.CheckpointStatusHistories.RemoveRange(checkpoint.StatusHistory);
